    ...
    if (!response.ok) {
      return {
        paths: [
          ...
          { params: { slug: "introducing-nano-collective-a-new-era-of-privacy-first-ai-101" } }
        ],
        fallback: false,
      };
    }
...
export const getStaticProps: GetStaticProps<BlogPostProps> = async ({
  ...
  const generateDummyPost = (): BlogPostProps => {
    if (slug === "introducing-nano-collective-a-new-era-of-privacy-first-ai-101") {
      return {
        post: {
          id: "dummy-2",
          number: 101,
    ...
    return {
      post: {
        ...
        bodyHTML: "<p>Nanocoder has hit 2000 stars on GitHub! This is a huge milestone for our privacy-first local AI development tool.</p><p>We want to thank our amazing community for the support, issues, and PRs. This validates our belief that developers want absolute control over their codebase without leaking IP to external APIs.</p><h2>What's Next?</h2><p>We are currently working on V2, which will include better context resolution and full integration with local LLMs via Ollama.</p>",
        excerpt: "Nanocoder has hit 2000 stars on GitHub! This is a huge milestone for our privacy-first local AI development tool...",
  ...
  try {
    ...
    if (!discussionsResponse.ok) {
      ...
      if (slug === "nanocoder-hit-2000-github-stars-50" || slug === "introducing-nano-collective-a-new-era-of-privacy-first-ai-101") {
        return { props: generateDummyPost() };
      }
      return { notFound: true };
    }
